...
SQL_QUERIES = {
    ...
    "link_not_found_ids": """
        ...
        {exclude_condition};
    """,
    "chamber_access_issue_count": """
        SELECT COUNT(cmnt_eta_c)
        FROM {table}
        WHERE cmnt_eta_c NOT IN ('RAS', 'GPS MODIFIES')
        AND cmnt_eta_c NOT ILIKE 'AUTRE%';
    """,
}
...
def gather_column_info(cursor, table_name, column_name):
    ...
    # Determine if we should exclude problematic chambers
    exclude_problematic = column_name != "c_pano_av"
    exclude_condition = "AND cmnt_eta_c IN ('RAS', 'GPS MODIFIES') OR cmnt_eta_c ILIKE 'AUTRE%'" if exclude_problematic else ""
    
    # Get counts
    for check_name in ["total_count", "file_not_found_count", "working_count"]:
        cursor.execute(SQL_QUERIES[check_name].format(col=column_name, table=table_name))
        column_info[check_name] = cursor.fetchone()[0]
    
    # Special handling for link_not_found queries
    cursor.execute(SQL_QUERIES["link_not_found_count"].format(
        col=column_name, 
        table=table_name,
        exclude_condition=exclude_condition
    ))
    column_info["link_not_found_count"] = cursor.fetchone()[0]
    
    # Get IDs for "File Not Found" (unchanged)
    ...
    # Get IDs for "Link Not Found" with condition
    cursor.execute(SQL_QUERIES["link_not_found_ids"].format(
        col=column_name, 
        table=table_name,
        exclude_condition=exclude_condition
    ))
...
def generate_report(conn, table_name):
    ...
    with conn.cursor() as cursor:
        # Get chamber access issue count
        cursor.execute(SQL_QUERIES["chamber_access_issue_count"].format(table=table_name))
        chamber_access_issue = cursor.fetchone()[0]
        
        # Add chamber access issue to report
        report["chamber_access_issue"] = chamber_access_issue
        
        # Get column info as before
        for col in columns_to_check:
            report[col] = gather_column_info(cursor, table_name, col)
    return report

def display_report_gui(report, folder_path):
    window = tk.Tk()
    ...
    text_area.insert(tk.END, f"Folder Path: {folder_path}\n", "spacing")
    text_area.insert(tk.END, f"Number of Files in Folder: {num_files}\n", "spacing")
    text_area.insert(tk.END, f"Chamber Access Issue: {report['chamber_access_issue']}\n", "spacing")
    
    # Only iterate through the columns we want to check, not the chamber_access_issue
    ...
    # Add a save button
    save_button = tk.Button(button_frame, text="Save Report", command=lambda: save_report(report, folder_path))
    save_button.pack(side=tk.LEFT, padx=10, pady=5)
    
    # Add a save as CSV button
    save_csv_button = tk.Button(button_frame, text="Save as CSV", command=lambda: save_csv_report(report, folder_path))
...
def save_report(report, folder_path):
    ...
    # Add folder information
    num_files = len([f for f in os.listdir(folder_path) if os.path.isfile(os.path.join(folder_path, f))])
    report_text += f"Folder Path: {folder_path}\n"
    report_text += f"Number of Files in Folder: {num_files}\n"
    ...
    if file_path.endswith(".txt"):
        ...
        doc.add_paragraph(f"Number of Files in Folder: {num_files}")
...
def save_csv_report(report, folder_path):
    """
    ...
    try:
        with open(file_path, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            csv_writer.writerow(["Folder Path", folder_path])
            csv_writer.writerow(["Number of Files in Folder", len([f for f in os.listdir(folder_path) if os.path.isfile(os.path.join(folder_path, f))])])
            csv_writer.writerow([])
            csv_writer.writerow(["Chamber Access Issue", report["chamber_access_issue"]])
            csv_writer.writerow([])
...
def main(dbname, user, password, host, port, table_name, folder_path):
    try:
        ...
        display_report_gui(report, folder_path)
        conn.close()
    except Exception as e:
...
if __name__ == "__main__":
    main()  # Call the main function with your parameters